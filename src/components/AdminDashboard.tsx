  ...
  return (
    <div className="max-w-[1480px] mx-auto px-6 lg:px-8 py-8 md:py-12">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
        ...
        <button onClick={handleLogout} className="px-6 py-2.5 rounded-xl font-bold text-[14px] text-rose-600 bg-rose-50 border border-rose-200 hover:bg-rose-100 hover:text-rose-700 transition-all outline-none focus:ring-2 focus:ring-rose-200 shrink-0">
          Keluar
        </button>
      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[1000] flex items-center justify-center p-4 sm:p-6 animate-[fadeIn_200ms_ease-out]" onClick={() => setInspectorOpen(false)}>
          <div className="bg-white rounded-[24px] shadow-2xl shadow-slate-900/10 w-full max-w-[560px] max-h-[90vh] flex flex-col animate-[slideUp_300ms_ease-out] relative overflow-hidden" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-8 overflow-y-auto">
              ...
              {/* Lock Controls */}
              {!inspectorData.booking && (inspectorData.status === 'available' || inspectorData.status === 'locked') && (
                <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200">
                  ...
                  {inspectorData.status === 'locked' ? (
                    ...
                    <div className="flex flex-col gap-4">
                      <div>
                        <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Keterangan Kunci (Opsional)</label>
                        <input 
                          type="text" 
                          ...
                          placeholder="misal: Maintenance, Rapat Prodi" 
                          value={lockNote} 
                          onChange={(e) => setLockNote(e.target.value)}
                        />
                      </div>
                      <button className="w-fit px-5 py-2.5 rounded-xl font-bold text-[13px] text-white bg-slate-800 hover:bg-slate-900 transition-colors shadow-md" onClick={() => handleToggleLock(true)}>Kunci Ruangan</button>
                    </div>
                  )}
                </div>
              )}
              ...
              {(inspectorData.status === 'scheduled' || inspectorData.status === 'borrowed') && (
                <div className="bg-amber-50/80 p-5 rounded-2xl border border-amber-200/60 flex gap-4 items-start">
                  ...
                  <div>
                    <h4 className="text-[14px] font-bold text-amber-900 mb-1">Jadwal Terkunci</h4>
                    <p className="text-[13px] text-amber-700/80 font-medium leading-relaxed">Ini adalah jadwal perkuliahan resmi atau sedang dipinjam oleh prodi lain. Anda tidak dapat mengubah data ini dari sistem SIMAR.</p>
                  </div>
                </div>
              )}
