...
export default function StatusPage() {
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!query.trim()) {
      ...
      setFilteredBookings(
        bookings.filter(b => 
          (b.nim && b.nim.toLowerCase().includes(lowerQuery)) || 
          b.id.toLowerCase().includes(lowerQuery) ||
          b.namaPJ.toLowerCase().includes(lowerQuery) ||
          b.namaMatakuliah.toLowerCase().includes(lowerQuery)
        )
      );
    }
  };
  ...
  return (
    <div className="max-w-[1480px] mx-auto px-6 lg:px-8 py-8 md:py-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
        <div>
          <h1 className="text-3xl md:text-[34px] font-extrabold text-slate-800 tracking-tight mb-2">Daftar Peminjaman Ruangan</h1>
          <p className="text-[15px] text-slate-500 leading-relaxed">
            Daftar seluruh pengajuan peminjaman ruangan oleh mahasiswa. Gunakan pencarian untuk menemukan data Anda.
          </p>
        </div>
      </div>
      
      <div className="relative w-full max-w-[560px] mb-10 group flex gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Cari ID Booking, NIM, Nama PJ, atau Keperluan..."
            value={searchQuery}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {isLoading ? (
          ...
          <div className="col-span-full glass-card flex flex-col items-center justify-center p-16 text-center">
            ...
            <h3 className="text-xl font-bold text-slate-800 mb-2">Tidak ada data peminjaman</h3>
            <p className="text-slate-500 max-w-sm">Belum ada request yang masuk atau pencarian Anda tidak cocok.</p>
          </div>
        ) : (
            ...
            <div key={b.id} className="glass-card p-6 flex flex-col relative overflow-hidden group hover:shadow-lg hover:-translate-y-1 transition-all border border-slate-200/60 bg-white">
              ...
              <div className="flex flex-col gap-3 bg-slate-50 rounded-xl p-4 border border-slate-100">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <span className="text-[13px] font-semibold text-slate-800 ml-5">{b.namaPJ}</span>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
